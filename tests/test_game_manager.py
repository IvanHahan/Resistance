...
class TestGameProgress(TestCase):
    ...
    def test_troop_proposal_success(self):
        with self.app.app_context():
            ...
            _ = game_manager.update_game(game, sid='1')
            ...
            self.assertTrue(len(game.current_mission().current_voting().votes) == len(game.players))
            self.assertTrue(len(game.current_mission().troop_proposals[-1].members) == 1)
            self.assertTrue(game.current_mission().troop_proposals[-1].members[0].id == 1)
    ...
    def test_troop_proposal_not_leader_fail(self):
        with self.app.app_context():
            ...
            with self.assertRaises(errors.NotLeader):
                game_manager.update_game(game, sid='2', players_ids=[1])
            self.assertTrue(game.current_mission().stage == model.RoundStage.troop_proposal)
    ...
    def test_troop_proposal_wrong_troop_size_fail(self):
        with self.app.app_context():
            ...
            with self.assertRaises(errors.InvalidPlayersNumber):
                game_manager.update_game(game, sid='3', players_ids=[1, 2])
            self.assertTrue(game.current_mission().stage == model.RoundStage.troop_proposal)
    ...
    def test_troop_vote_approve_success(self):
        with self.app.app_context():
            ...
            self.assertTrue(game.current_mission().stage == model.RoundStage.troop_voting_results)
            self.assertTrue(game.current_mission().troop_proposals[-1].voting.result is True)
    ...
    def test_troop_vote_approve_twice_vote_fail(self):
        with self.app.app_context():
            ...
            with self.assertRaises(errors.CantVote):
                game_manager.update_game(game, sid='2', result=True)
            self.assertTrue(game.current_mission().stage == model.RoundStage.troop_voting)
            self.assertTrue(game.current_mission().troop_proposals[-1].voting.result is None)
    ...
    def test_troop_vote_disapprove_success(self):
        with self.app.app_context():
            ...
            self.assertTrue(game.current_mission().stage == model.RoundStage.proposal_request)
            self.assertTrue(game.current_mission().troop_proposals[-1].voting.result is False)
